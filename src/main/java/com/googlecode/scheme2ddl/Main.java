...
public class Main {
	...
	private static boolean cleanOutputDirectory = false;
	...
	public static void main(String[] args) throws Exception {
		...
		if (justTestConnection) {
			...
			new UserObjectJobRunner().start(context, isLaunchedByDBA,
					objectFilter.toLowerCase(), typeFilter.toUpperCase(),
					typeFilterMode.toLowerCase());
		}
	}

	private static void testDBConnection(ConfigurableApplicationContext context)
			throws SQLException {
		ConnectionDao connectionDao = (ConnectionDao) context
				.getBean("connectionDao");
		OracleDataSource dataSource = (OracleDataSource) context
				.getBean("dataSource");
		if (connectionDao.isConnectionAvailable()) {
			System.out.println("OK success connection to "
					+ dataSource.getURL());
		} else {
	...
	private static void modifyContext(ConfigurableApplicationContext context) {
		if (dbUrl != null) {
			...
			// for OracleDataSource in connectionCachingEnabled mode need
			// explicitly set user and password
		...
		if (cleanOutputDirectory)
			writer.setCleanOutputDirectory(cleanOutputDirectory);
		...
		if (parallelCount > 0) {
			SimpleAsyncTaskExecutor taskExecutor = (SimpleAsyncTaskExecutor) context
					.getBean("taskExecutor");
			taskExecutor.setConcurrencyLimit(parallelCount);
		}
		String userName = ((OracleDataSource) context.getBean("dataSource"))
				.getUser();
		isLaunchedByDBA = userName.toLowerCase().matches(".+as +sysdba *");
		if (!isLaunchedByDBA) {
			ConnectionDao connectionDao = (ConnectionDao) context
					.getBean("connectionDao");
			isLaunchedByDBA = connectionDao.hasSelectCatalogRole(); // todo
																	// rename
																	// isLaunchedByDBA
																	// ->
																	// processForeignSchema
		}
		// process schemas
		processSchemas(context);

		FileNameConstructor fileNameConstructor = retrieveFileNameConstructor(context); // will
																						// create
																						// new
																						// one
																						// if
																						...
																						// exist
		if (isLaunchedByDBA) {
			fileNameConstructor.setTemplate(fileNameConstructor
					.getTemplateForSysDBA());
			fileNameConstructor.afterPropertiesSet();
		}

		if (stopOnWarning) {
			UserObjectProcessor processor = (UserObjectProcessor) context
					.getBean("processor");
	...
	private static void processSchemas(ConfigurableApplicationContext context) {
		...
		if (listFromContext.size() == 1) {
			try {
				UserObjectReader userObjectReader = (UserObjectReader) context
						.getBean("reader");
	...
	private static void fillSchemaListFromUserName(
			ConfigurableApplicationContext context) {
		OracleDataSource dataSource = (OracleDataSource) context
				.getBean("dataSource");
		String schemaName = dataSource.getUser().split(" ")[0];
	...
	/**
	 * @param context
	 * @return existing bean 'schemaList', if this exists, or create and
	 *         register new bean
	 */
	private static List<String> retrieveSchemaListFromContext(
		...
		try {
			...
			DefaultListableBeanFactory beanFactory = (DefaultListableBeanFactory) context
					.getBeanFactory();
			beanFactory.registerBeanDefinition("schemaList",
					BeanDefinitionBuilder.rootBeanDefinition(ArrayList.class)
							.getBeanDefinition());
			list = (List) context.getBean("schemaList");
		}
		return list;
	...
	/**
	 * @param context
	 * @return existing bean 'fileNameConstructor', if this exists, or create
	 *         and register new bean
	 */
	private static FileNameConstructor retrieveFileNameConstructor(
			ConfigurableApplicationContext context) {
		FileNameConstructor fileNameConstructor;
		try {
			fileNameConstructor = (FileNameConstructor) context
					.getBean("fileNameConstructor");
		} catch (NoSuchBeanDefinitionException e) {
			DefaultListableBeanFactory beanFactory = (DefaultListableBeanFactory) context
					.getBeanFactory();
			...
			fileNameConstructor = (FileNameConstructor) context
					.getBean("fileNameConstructor");
			fileNameConstructor.afterPropertiesSet();
			// for compatability with old config without fileNameConstructor
			// bean
			UserObjectProcessor userObjectProcessor = (UserObjectProcessor) context
					.getBean("processor");
	...
	private static void validateContext(ConfigurableApplicationContext context) {
		...
		if (!isLaunchedByDBA) {
			...
			Assert.state(
					userName.startsWith(schemaName),
					String.format(
	...
	private static void printUsage() {
		...
		msg.append("util for export oracle schema from DB to DDL scripts (file per object)"
				+ lSep);
		...
		msg.append("                             example: scott/tiger@localhost:1521:ORCL"
				+ lSep);

		msg.append("  -o, --output,              output dir" + lSep);
		msg.append("  -e, --encoding,            output encoding" + lSep);
		msg.append("  --clean,                   clean output directory" + lSep);
		msg.append("  -p, --parallel,            number of parallel thread (default 4)"
				+ lSep);
		msg.append("  -s, --schemas,             a comma separated list of schemas for processing"
				+ lSep);
		msg.append("                             (works only if connected to oracle as sysdba)"
				+ lSep);
		msg.append("  -c, --config,              path to scheme2ddl config file (xml)"
				+ lSep);
		msg.append("  -f, --filter,              filter for specific DDL objects"
				+ lSep);
		msg.append("                             every LIKE wildcard can be used"
				+ lSep);
		msg.append("  -tf, --type-filter,        filter for specific DDL object types"
				+ lSep);
		msg.append("  -tfm, --type-filtermode,   mode for type filter: include(default) or exclude"
				+ lSep);
		msg.append("  --stop-on-warning,         stop on getting DDL error (skip by default)"
				+ lSep);
		msg.append("  -rsv,                      replace actual sequence values with 1 "
				+ lSep);
		msg.append("  --replace-sequence-values, " + lSep);
		msg.append("  -tc,--test-connection,     test db connection available"
				+ lSep);
		msg.append("  -version,                  print version info and exit"
				+ lSep);
	...
	private static void collectArgs(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (arg.equals("-help") || arg.equals("-h") || arg.equals("--help")) {
				...
				i++;
			} else if (arg.equals("-s") || arg.equals("-schemas")
					|| arg.equals("--schemas")) {
				schemas = args[i + 1];
				i++;
				...
				stopOnWarning = true;
			} else if ((arg.equals("-fsv") || arg
					.equals("--filter-sequence-values"))) {
				...
				i++;
				// default to include if anything except include or exclude is
				// given as argument
				if (!typeFilterMode.equals("include")
						&& !typeFilterMode.equals("exclude")) {
